...
instruction.stringArg = line;  // Assign the trimmed line to instruction.stringArg
...
// value, etc.)
    ...
    PcbEntry& nextProcess = pcbEntry[nextProcessId];
    nextProcess.state = STATE_RUNNING;
    cpu.pProgram = &nextProcess.program;  // Pointer to the program of the process
    cpu.programCounter = nextProcess.programCounter;  // Start of the program counter
    cpu.value = nextProcess.value;  // Specific value to CPU from process
...
// 3. Update the running state to -1 (basically mark no process as running). Note that a new process will be chosen to run later (via the Q command code calling the schedule() function).
    ...
    // 1. Add the PCB index of the running process (stored in runningState) to the blocked queue.
    if (runningState != -1) {  // Ensure there is a running process to block
...
// 4. Update the running state to -1 (basically mark no process as running). Note that a new process will be chosen to run later (via the Q command code calling the schedule function).
    ...
    // 1. Get the PCB entry of the running process.
    if (runningState != -1) {  // Ensure there is a running process to end
...
// 6. Increment the cpu's program counter by the value read in #3
    ...
    int freeIndex = -1;
    for (int i = 0; i < 10; i++) {  // Assuming the size of pcbEntry is 10
        if (pcbEntry[i].state == STATE_READY || pcbEntry[i].state == STATE_BLOCKED) {
            continue;  // Skip entries that are actively in use
        }
        freeIndex = i;
        break;
    }
    ...
    if (freeIndex == -1) {
        ...
        return;  // No free PCB entry was found
    }
    ...
    // 3. Ensure the passed-in value is not out of bounds.
    if (value < 0 || value >= 10) {  // Assuming program size limit
        cout << "Value is out of bounds." << endl;
        return;
    }

    // 4. Populate the PCB entry obtained in #1
    pcbEntry[freeIndex].processId = freeIndex;  // a. Set the process ID to the PCB index obtained in #1.
    pcbEntry[freeIndex].parentProcessId = parentProcess.processId;  // b. Set the parent process ID
    pcbEntry[freeIndex].programCounter = cpu.programCounter;  // c. Set the program counter to the cpu program counter.
    pcbEntry[freeIndex].value = cpu.value;  // d. Set the value to the cpu value.
    pcbEntry[freeIndex].priority = parentProcess.priority;  // e. Set the priority to the same as the parent's.
    pcbEntry[freeIndex].state = STATE_READY;  // f. Set the state to the ready state.
    pcbEntry[freeIndex].startTime = timestamp;  // g. Set the start time to the current timestamp
...
// 3. Set the program counter to 0.
    ...
    // 2. Use createProgram() to read in the filename specified by the argument into the CPU
